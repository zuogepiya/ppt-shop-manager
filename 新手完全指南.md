...
3. 向下滚动页面，找到 **Connection parameters** 或 **Connection info** 部分

**如果你看到的是 "Connection pooling configuration"**：

- 向下滚动更多，寻找其他选项
- 或者点击页面上的其他标签页（如 General、Connection Info 等）
- 通常 "Connection string" 会在页面中间或靠下的位置
...
8. ⚠️ **务必保存这个完整的连接字符串！记事本或其他地方保存好！**

---

**方法二：通过Project API部分（备用方案）**

如果在Database设置中找不到，可以尝试这个方法：

1. 在左侧菜单栏，点击 **Settings** 图标（⚙️）
2. 选择 **API**
3. 在 **Project URL** 和 **API Keys** 部分，找到 **Connection string**
4. 复制连接字符串并替换密码
...
如果以上方法都找不到，可以手动构建连接字符串：

**步骤1：找到项目ID**

1. 在Supabase Dashboard的URL中找到项目ID
...
**⚠️ 重要提示**：
- 连接字符串中的 `6543` 是端口号，保持不变
- `aws-0-ap-northeast-1.pooler.supabase.com` 是服务器地址，根据你的项目可能会有所不同（如 `ap-northeast-1`、`us-east-1` 等）
- 最重要的是：项目ID 和 密码 必须正确
...
**情况C：提示需要身份验证**
- 可能需要输入密码或Personal Access Token
- 如果提示需要Token，可以按照提示操作
